// Floating astronaut chess players. One holds a tablet showing the official
// TDC logo (supplied asset, displayed as-is) which projects their hovering
// chessboard; the other plays with a free arm (thinking pose, thumbs-up).
...
export function createAstronaut({ logoUrl, tablet = true, skin = '#8C5A3C', hair = '#2A1C16', patch = '#D8B15E' }) {
  const SKIN = skin;
  const id = uid('astro');
  const markup = `
  <defs>
  ...
  <g data-part="body">
    ...
    <g transform="translate(-29 -86)">
      <circle r="8" fill="#0B1E3D" stroke="${patch}" stroke-width="1.5"/>
      <path d="${PIECE_PATHS.knight}" transform="scale(.11) translate(-50 -66)" fill="#D8B15E"/>
    </g>
    ${tablet ? `<path d="M-46 -92 Q-64 -60 -44 -40 L-14 -40 L-16 -52 L-34 -54 Q-36 -72 -30 -86 Z" fill="url(#${id}-suit)" stroke="#9AA6BB" stroke-width="1.5"/>
    <path d="M46 -92 Q64 -60 44 -40 L14 -40 L16 -52 L34 -54 Q36 -72 30 -86 Z" fill="url(#${id}-suit)" stroke="#9AA6BB" stroke-width="1.5"/>` : `
    <path d="M-46 -92 Q-66 -60 -60 -24 Q-58 -4 -44 0 L-34 -10 Q-44 -30 -40 -60 Q-38 -76 -30 -86 Z" fill="url(#${id}-suit)" stroke="#9AA6BB" stroke-width="1.5"/>
    <circle cx="-44" cy="0" r="9.5" fill="#F4F6FA" stroke="#9AA6BB" stroke-width="1.4"/>
    <g data-part="armR">
      <path d="M30 -98 Q52 -106 60 -86 L71 -50 Q63 -36 50 -44 Z" fill="url(#${id}-suit)" stroke="#9AA6BB" stroke-width="1.5"/>
      <g data-part="foreR">
        <path d="M71 -52 Q60 -34 49 -44 L21 -90 Q25 -106 38 -102 Z" fill="url(#${id}-suit)" stroke="#9AA6BB" stroke-width="1.5"/>
    ...
    <g data-part="tablet" ${tablet ? '' : 'display="none"'}>
      ...
      <g clip-path="url(#${id}-screen)">
        ${logoUrl ? `<image href="${logoUrl}" x="-33" y="-63" width="66" height="38" preserveAspectRatio="xMidYMid meet"/>` : ''}
      </g>
    ...
    <g data-part="head">
      ...
      <g clip-path="url(#${id}-visorClip)">
        ...
        <path d="M-22 -150 Q-24 -176 0 -178 Q24 -176 22 -150 Q16 -166 0 -166 Q-16 -166 -22 -150 Z" fill="${hair}"/>
        <g data-part="features">
          ...
          <path data-part="browL" d="M-12 -157 Q-8 -160 -4 -158" stroke="${hair}" stroke-width="1.8" fill="none" stroke-linecap="round"/>
          <path data-part="browR" d="M4 -158 Q8 -160 12 -157" stroke="${hair}" stroke-width="1.8" fill="none" stroke-linecap="round"/>
          ...
          <ellipse data-part="mO" cx="0" cy="-133" rx="3" ry="3.6" fill="${MOUTH}" opacity="0"/>
        </g>
  ...
  const rig = new Rig({
    root,
    parts: p,
    joints: { head: [0, -108], body: [0, -40], tablet: [0, -44], armR: [42, -94], foreR: [60, -46] },
    face: {
      eyes: [0, 1].map((i) => ({ lid: p[`lid${i}`], lash: p[`lash${i}`], iris: p[`iris${i}`], ry: 4 })),
      brows: [{ node: p.browL, cx: -8, cy: -158 }, { node: p.browR, cx: 8, cy: -158 }],
      mouths: { mSoft: p.mSoft, mGrin: p.mGrin, mO: p.mO },
      features: p.features,
      turnShift: 3,
      maxLook: [1, 1.2],
    },
    state: { lookY: 0.8, lid: 0.25, mSoft: 1, mGrin: 0, mO: 0, tap: 0, thumb: 0 },
  });
  ...
  rig.onApply((s) => {
    ...
    if (p.thumb) setOpacity(p.thumb, s.thumb);
  });
  return rig;
}
